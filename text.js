...
function myajax0(method,url,data,callback,type) {
    var xhr;
    ...
    if(JSON.stringify(data) != '{}'){
        url += '?';
        for(var i in data){
            estimatedata += i+'='+data[i]+'&';   //将js对象重组，拼接成url参数存入estimatedata变量中
        }
        //使用slice函数提取一部分字符串，这里主要是为了去除拼接的最后一个&字符
        //slice函数：返回一个新的字符串。包括字符串从 start 开始（包括 start）到 end 结束（不包括 end）为止的所有字符。
        estimatedata = estimatedata.slice(0,estimatedata.length-1);
    }

    //判断method是否为get
    if(method == "get"){
        //是则将数据拼接在url后面
        url = url+estimatedata;
    }
